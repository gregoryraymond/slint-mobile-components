//! Page templates in the `system` category, plus snapshot-test scene
//! wrappers (`Snap*Page` Windows) for every page. Consumers wire in
//! the `.slint` sources via the `mobile-pages-system` library_paths alias.
...
/// workspace root can `pub use slint_mobile_pages_system::scenes::*` to
...
pub mod scenes {
    pub use crate::_generated_snapshot_scenes::{
        ...
        SnapHelpCenterPage, SnapNotificationCenterPage, SnapSecurityCheckupPage,
        SnapStorageManagerPage, SnapWiFiSettingsPage,
    };
}
