//! Page templates in the `media` category, plus snapshot-test scene
//! wrappers (`Snap*Page` Windows) for every page. Consumers wire in
//! the `.slint` sources via the `mobile-pages-media` library_paths alias.
...
/// workspace root can `pub use slint_mobile_pages_media::scenes::*` to
...
pub mod scenes {
    pub use crate::_generated_snapshot_scenes::{
        ...
        SnapEditorialArticlePage, SnapEqualizerPage, SnapLiveStreamPage,
        SnapMediaPlayerLockscreenPage, SnapMusicLibraryPage, SnapPhotoGridPage,
        SnapPhotoViewerPage, SnapPlaylistDetailPage, SnapPodcastPage, SnapVaporwavePlayerPage,
