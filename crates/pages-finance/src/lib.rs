//! Page templates in the `finance` category, plus snapshot-test scene
//! wrappers (`Snap*Page` Windows) for every page. Consumers wire in
//! the `.slint` sources via the `mobile-pages-finance` library_paths alias.
...
/// workspace root can `pub use slint_mobile_pages_finance::scenes::*` to
