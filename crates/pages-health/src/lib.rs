//! Page templates in the `health` category, plus snapshot-test scene
//! wrappers (`Snap*Page` Windows) for every page. Consumers wire in
//! the `.slint` sources via the `mobile-pages-health` library_paths alias.
...
/// workspace root can `pub use slint_mobile_pages_health::scenes::*` to
